...
FeatherMap::FeatherMap():has_base(0),ddice(0),n_samp(0),has_guide(0),guide_data(0),bind_data(0),guide_spaceinv(0),
...
void FeatherMap::setBase(const MObject& mesh)
{
	obase = mesh;
	has_base = 1;
}

void FeatherMap::setGuide(const MObjectArray& meshes)
{
	oguide.clear();
...
// find per-patch number of segment
	...
	if(guide_data) delete[] guide_data;
	guide_data = new AGuider[num_guide];
...
int FeatherMap::save(const char* filename)
{
	ofstream outfile;
...
int FeatherMap::saveStart(const char* filename)
{
	ofstream outfile;
	outfile.open(filename, ios_base::out | ios_base::binary);
	if(!outfile.is_open()) 
...
int FeatherMap::load(const char* filename)
{
	ifstream infile;
...
int FeatherMap::loadStart(const char* filename)
{
	ifstream infile;
	infile.open(filename, ios_base::in | ios_base::binary);
	if(!infile.is_open()) 
