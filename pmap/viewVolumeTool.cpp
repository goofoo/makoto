...
/*
MStatus viewVolumeContext::doDrag( MEvent & event )
//
// Drag out the viewVolume (using OpenGL)
//
{
...
MStatus viewVolumeContext::doRelease( MEvent & event )
//
// Selects objects within the viewVolume box.
{MGlobal::displayInfo("view volume release");

	MSelectionList			incomingList, viewVolumeList;

	// Create the viewVolume when you release the mouse button
	//if(mOpt==0) grow();


	// If HW overlays enabled, then clear the overlay plane
	// such that the viewVolume is no longer drawn on screen.

	// Get the end position of the viewVolume
	event.getPosition( last_x, last_y );
	...
	// it back.
		...
		if ( abs(start_x - last_x) < 2 && abs(start_y - last_y) < 2 ) {
			...
			// Select all the objects or components within the viewVolume.
			MGlobal::selectFromScreen( start_x, start_y, last_x, last_y,
									   MGlobal::kReplaceList );
		}

		// Get the list of selected items
		MGlobal::getActiveSelectionList(viewVolumeList);
		...
		MGlobal::selectCommand(viewVolumeList, listAdjustment);
	}
	return MS::kSuccess;		
}
*/
...
/*
void viewVolumeContext::setOperation(unsigned val)
{
	mOpt = val;
	MToolsInfo::setDirtyFlag(*this);
}

unsigned viewVolumeContext::getOperation() const
{
	return mOpt;
}

void viewVolumeContext::setNSegment(unsigned val)
{
	m_numSeg = val;
	MToolsInfo::setDirtyFlag(*this);
}

unsigned viewVolumeContext::getNSegment() const
{
	return m_numSeg;
}

void viewVolumeContext::setLSegment(float val)
{
	m_seg_len = val;
	MToolsInfo::setDirtyFlag(*this);
}

float viewVolumeContext::getLSegment() const
{
	return m_seg_len;
}

void viewVolumeContext::setCreationNoise(float val)
{
	m_seg_noise = val;
	MToolsInfo::setDirtyFlag(*this);
}

float viewVolumeContext::getCreationNoise() const
{
	return m_seg_noise;
}
*/
/*
void viewVolumeContext::doCollide(const MPoint& vert, MVector &v)
{
...
void viewVolumeContext::toClosest(const MPoint& vert, MVector &v)
{
	MPoint moveto = vert + v;
	MPoint closestP;
	MVector closestN;
...
void viewVolumeContext::moveAll()
{
	MPoint toNear, toFar;
...
void viewVolumeContext::dragTip()
{
	MPoint toNear, toFar;
	view.viewToWorld ( last_x, last_y, toNear, toFar );
...
void viewVolumeContext::scaleAll()
{
...
void viewVolumeContext::dragRoot()
{
	MPoint toNear, toFar;
	view.viewToWorld ( last_x, last_y, toNear, toFar );
