...
void rockingTransformMatrix::setRockInX( double rock, const MString& scene, const MString& mesh)
{
	...
	double delta = rock - zGlobal::safeConvertToInt(rock);
	
	std::string sbuf(scene.asChar());
	zGlobal::changeFrameNumber(sbuf, zGlobal::safeConvertToInt(rock));
	...
	if(delta >0)
	{
		zGlobal::changeFrameNumber(sbuf, zGlobal::safeConvertToInt(rock+1.0));
