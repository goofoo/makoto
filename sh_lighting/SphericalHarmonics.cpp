...
sphericalHarmonics::sphericalHarmonics(void):m_pSample(0)
{
	...
	setupSampleStratified(m_pSample, SH_N_SQRT_SAMPLES);
	
	ConstantSph cl;
	projectFunction(cl, constantCoeff);
}
...
void sphericalHarmonics::setupSampleStratified(SHSample* samp, const int n)
{
	...
	for(int j=0, k=0; j<n; j++)
	{
		for(int i=0; i<n; i++, k++)
		{
			fx = noi.randfint(i*13+j*7);
			fy = noi.randfint(i*15+j);
			...
			double x = (i+fx) * oneovern;
			double y = (j+fy) * oneovern;
...
struct rgbReconstructSH : public SHFunction
{
	...
	float getFloat(const SHSample& s) const
	{
		...
		for(int i=0; i<SH_N_BASES;i++) value += (coeffs[i].x + coeffs[i].y + coeffs[i].z) *s.coe[i];
...
void sphericalHarmonics::addCoeff(XYZ* a, XYZ* b)
{
	for (int j = 0; j < SH_N_BASES; j++) {
		a[j].x += b[j].x;
		a[j].y += b[j].y;
