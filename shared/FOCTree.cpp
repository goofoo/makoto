...
1.define global variable :
  ...
  int sizegainarray = 0;  
  XYZ val;
2.create FOctree object ，example：FOctree b；
3.create TreeNode object ，example：TreeNode *t = new TreeNode(0,size - 1);
4.create BST tree，example：b.CreateFOctree(t,array,0,size - 1);
5.define determine point，example：val.x = 50.0;val.y = 50.0;val.z = 50.0;
6.search determine point's around，example：
  sizegainarray = 0;
  b.SearchFOctree(t,val,range);
  （attention：every search ,use sizegainarray = 0）
7.重复5-6，搜索不同点。
*/

void FOctree::FreeTreeNode(TreeNode *p)
{
	delete p;
}

TreeNode* FOctree::GetTreeNode(int item1, int item2, TreeNode *lptr = 0, TreeNode *rptr = 0)
{
...
int FOctree::FOctreeDepth()
{
	return Depth(tree);
}


void FOctree::quick_sortX(XYZ array[],int first,int last)
{
...
void FOctree::quick_sortY(XYZ array[],int first,int last)
{
	int low,high;float list_separator;
	XYZ temp;
...
void FOctree::quick_sortZ(XYZ array[],int first,int last)
{
	int low,high;float list_separator;
	XYZ temp;
...
void FOctree::CreateFOctree( TreeNode *t,XYZ array[],int begin,int end)
{
	if(tree == NULL )
	{
		int i ;
		...
		CreateFOctree(t->left,array,begin,i-1);
		CreateFOctree(t->right,array,i,end);
	}
	else 
	if((end - begin)>=6)
	{
...
void FOctree::SearchFOctree( TreeNode *t,XYZ val,int range)
{
	float middle;
	if(t==NULL){
		t = tree;
	...
	if(t->flag == -1)
	{
		...
		if((t->min<=val.x+range)&&(val.x-range<=t->max))
		{
			...
			{
				SearchFOctree( t->left,val,range );
				SearchFOctree( t->right,val,range );
			}
...
void FOctree::PrintFOctree(TreeNode *t)
{
	if(t!=NULL)
	{
		cout<<"begin:"<<t->begin<<" end:"<<t->end<<endl;
		PrintFOctree(t->left);
		PrintFOctree(t->right);
	}
}