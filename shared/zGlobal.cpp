...
void zGlobal::trimByFirstSpace(std::string& res)
{
...
void zGlobal::getTime(std::string& log)
{
	time_t rawtime;
	struct tm * timeinfo;
...
void zGlobal::cutByFirstDot(std::string& res)
{
	int found = res.find('.', 0);
	if(found < 0) return;
...
void zGlobal::cutByLastDot(std::string& res)
{
...
void zGlobal::cutByLastSlash(std::string& res)
{
	int found = res.rfind('\\', res.size());
	if(found > 1) 
	{
...
void zGlobal::changeFrameNumber(std::string& res, int frame)
{
...
int zGlobal::safeConvertToInt(const double a)
{
	double b = (a*100.0 + 0.5)/100.0;
	return (int)b;
}

int zGlobal::getFrameNumber(std::string& name)
{
...
int zGlobal::compareFilenameExtension(std::string& name, const char* other)
{
...
void zGlobal::setFrameNumberAndExtension(std::string& name, int frame, const char* extension)
{
...
int zGlobal::getFrameNumber(const std::string& name)
{
...
void zGlobal::changeFilenameExtension(std::string& name, const char* ext)
{
	int found = name.find_last_of('.', name.size()-1);
	if(found < 0) return;
	found++;
...
void zGlobal::validateFilePath(std::string& name)
{
	std::string str(name);
...
void zGlobal::replacefilename(std::string& res, std::string& toreplace)
{
	int founddot;
...
void zGlobal::cutfilepath(std::string& res)
{
	int found = res.rfind('\\', res.size());
	if(found > 1) 
	{
