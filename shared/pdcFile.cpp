...
int pdcFile::readPositionAndAge(MVectorArray& positions, MVectorArray& velocities, MDoubleArray& ages)
{
	positions.clear();
	velocities.clear();
	ages.clear();
	...
	for (int i=0; i<pheader->numAttributes; i++ )
	{
		...
		//MGlobal::displayInfo(MString("type ") + attribType);
			...
			switch ( attribType )
			{
				...
				case 3: // Double Array,Float Array
					if(MString(attribName) == MString("age"))
					{
						for(int j=0; j<pheader->numParticles; j++)
						{
							if(pheader->byteOrder!=0) swapDouble( record );
							memcpy(&ages[j], record, sizeof(double));
							record += 8; //Move record pointer to next element
						}
					}
					else record += 8*pheader->numParticles;
					break;
